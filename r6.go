...
func processChunkR6(
	...
	for {
		...
		stats, ok := stationStats[row[0]]
		if !ok {
			stationStats[row[0]] = &Stats{
				Max:   reading,
				Min:   reading,
				Sum:   reading,
				Count: 1,
...
// Only difference is that we use *Stats instead of Stats and uglify the code a
// tiny bit. The next optimizations would be implementing a different map and/or
// optimizing the float conversion, which I'm not sure I'd be willing to do.
func r6(measurementsPath string) {
	file, err := os.Open(measurementsPath)
	if err != nil {
		log.Fatalf("Could not open %s: %s", measurementsPath, err)
	}
